...
def sigcomm_example_1_1():
    """
        example_1_1
        "There is no VNH for now"
        B and C default policies (no SDX specific policies)
        Advertised {C:{p1,p2,p3,p4}, B:{p1,p2,p3}}
        BGP best path {B:{p3}, C:{p1,p2,p4}}
        A's policy:
            match(dstport = 80) >> fwd(B), match(dstport = 443) >> fwd(C)
    
    """
    P_A = (((match(dstport = 80)) >> 
           ((match(dstip = prefixes['p1'])|match(dstip = prefixes['p2'])|
             match(dstip = prefixes['p4']))) >> 
            fwd(6)) +
            ((match(dstport = 443)) >> 
           ((match(dstip = prefixes['p1'])|match(dstip = prefixes['p2'])|
    ...
    A_match_1 = (intersection([(match(dstport = 80)), 
                 ((match(dstip = prefixes['p1'])|
                   match(dstip = prefixes['p2'])|
                   match(dstip = prefixes['p4'])))])
                 )
    
    A_match_2 = (intersection([(match(dstport = 443)), 
                 ((match(dstip = prefixes['p1'])|
                   match(dstip = prefixes['p2'])|
                   match(dstip = prefixes['p3'])|
                   match(dstip = prefixes['p4'])))])
                 )
    
    big_match_sdx_A = (A_match_1 | A_match_2)
    
    default_A = (if_((match(outport=5)), 
                      (modify(dstmac = mac_a1) >>fwd(1)), 
                      ((match(dstmac = mac_b1) >> fwd(6)) + 
                       (match(dstmac = mac_b2) >> fwd(7))+
                       (match(dstmac = mac_c1) >> fwd(8)))
    ...
    default_B = (if_((match(outport=6)), 
                      (modify(dstmac = mac_b1) >>fwd(2)), 
                      if_((match(outport=7)),
                          (modify(dstmac = mac_b2) >>fwd(3)),                         
                          ((match(dstmac = mac_a1) >> fwd(5)) +
                           (match(dstmac = mac_c1) >> fwd(8))
                           )                      
                        )
                     )
    ...
    default_C = (if_((match(outport=8)), 
                      (modify(dstmac = mac_c1) >>fwd(4)), 
                      ((match(dstmac = mac_b1) >> fwd(6)) + 
                       (match(dstmac = mac_b2) >> fwd(7))+
                       (match(dstmac = mac_a1) >> fwd(5)))
    ...
    disjoint_policies.append(drop + ((match(inport = 1)) >> augmented_A >> 
    ...
    disjoint_policies.append(drop + ((match(inport = 2)| match(inport = 3)) >> augmented_B >> 
                             match(outport = 5) >> augmented_A >> (match(outport = 1)))) 

    disjoint_policies.append(drop + ((match(inport = 2)| match(inport = 3)) >> augmented_B >> 
                             match(outport = 8) >> augmented_C >> (match(outport = 4))))
    
    disjoint_policies.append(drop + ((match(inport = 4)) >> augmented_C >> 
                             match(outport = 5) >> augmented_A >> (match(outport = 1)))) 
    ...
    #composed_policy = disjoint([augmented_A], [])
