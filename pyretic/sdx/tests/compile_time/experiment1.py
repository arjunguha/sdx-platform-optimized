...
def main():
    """
    policy1=((match(dstport=80) >> modify(dstport=8080)>>fwd(4))+
             (match(dstport=22)>> fwd(5))
             )
    policy2=((match(dstport=80) >> modify(dstport=8080)>>fwd(4))+
             (match(dstport=22)>> fwd(5))
             )
    hash1=policy1.__repr__()
    hash2=policy1.__repr__()
    if hash1==hash2:
        print "hash works"
    print policy2
    ...
    policy1=((match(dstport=80) >> modify(state='Test'))>>
             (match(state='Test')>>modify(state=None)>>fwd(5))
             )
    print policy1.compile()
    ...
    #print aggr_policies
    start_comp=time.time()
    aggr_policies.compile()
    
    
    #print "DATA: policy_parl",policy_parl
    #print "DATA: policy_seq",policy_seq
    print  ' Second run Completed Aggregate Compilation ',time.time() - start_comp, "seconds"
    ...
    #print "DATA: policy_parl",policy_parl
    #print "DATA: policy_seq",policy_seq
    print  'Run 2 Completed Aggregate Compilation ',time.time() - start_comp, "seconds"
