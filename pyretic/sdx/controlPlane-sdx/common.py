    ...
    for ind in range(1, ntot + 1):
        ...
        if ind <= nin:
            for i in range(nports):
                ...
                ip, mac = str(iplist[count]), str(
                    EUI(int(EUI(macinit)) + count))
                ports.append({'Id': count, 'MAC': mac, 'IP': ip})
    ...
    for participant in sdx.participants:
        ...
        if int(participant.id_) <= nin:
            ...
            for pfx in sdx.prefixes.keys():
                participant_to_ebgp_nh_received[
                    participant.id_][pfx] = random.randint(
                    1,
                    nin)
        participant_2_port[participant.id_] = {}
        participant_2_port[participant.id_][participant.id_] = [
            participant.phys_ports[i].id_ for i in range(len(participant.phys_ports))]
        for phyport in participant.phys_ports:
            port_2_participant[phyport.id_] = participant.id_
        for peer in participant.peers:
            participant_2_port[
                participant.id_][peer] = [
                participant.peers[peer].participant.phys_ports[0].id_]
    if debug:
...
def generate_policies(sdx, participants, ntot, nin):
    ...
    for participant_name in participants:
        ...
        participants[participant_name].policies = post_VNH(
            participants[participant_name].policies,
            sdx,
            participant_name)
